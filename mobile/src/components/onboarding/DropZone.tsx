  ...
  return (
    <Pressable onPress={handlePress} onPressIn={() => setPressed(true)} onPressOut={() => setPressed(false)}>
      ...
      >
        ...
        <Text className="mb-1.5 text-center text-17 font-sans-semibold text-white">
          Tap to choose your Spotify JSON files
        </Text>
        <Text className="mb-5 text-center text-13 text-white/45">Multiple files supported</Text>
        <View className="flex-row items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
          ...
          <Text className="text-11 font-sans-medium text-white/60">StreamingHistory_*.json</Text>
        </View>
      </MotiView>
    </Pressable>
  );
