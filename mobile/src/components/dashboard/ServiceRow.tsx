    ...
    <GlassCard padding="sm" rounded="xl">
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1">
          <Text className="text-[13px] text-white/40">{title}</Text>
          <Text numberOfLines={1} className="mt-0.5 text-[14px] font-sans-semibold text-white">
            {value}
          </Text>
        </View>
        <View
          className="h-2.5 w-2.5 rounded-full"
          style={{ backgroundColor: active ? colors.echoGreen : "rgba(255,255,255,0.2)" }}
        />
      </View>
    </GlassCard>
  );
