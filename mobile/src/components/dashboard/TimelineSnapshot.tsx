...
function ArtistBadge({ name, imageUrl, color, size }: { name: string; imageUrl?: string; color: string; size: number }) {
  ...
  return (
    ...
    >
      <Text className="text-[12px] font-sans-bold text-white">{initials}</Text>
    </LinearGradient>
  );
}
  ...
  return (
    <MotiView key={period.id} from={{ opacity: 0, translateY: 16 }} animate={{ opacity: 1, translateY: 0 }} transition={{ duration: 350 }}>
      <GlassCard padding="lg" rounded="2xl">
        <Text className="text-[10px] uppercase tracking-widest2 text-white/35">Snapshot</Text>
        ...
        <View className="mt-5 flex-row gap-2.5">
          <View className="flex-1 rounded-xl border border-white/5 bg-white/[0.03] p-3">
            <Text className="text-[9px] uppercase tracking-widest2 text-white/35">Top artist</Text>
            <View className="mt-2 flex-row items-center gap-2">
              <ArtistBadge name={period.topArtist} imageUrl={period.topArtistImageUrl} color={period.topArtistColor} size={28} />
              <Text numberOfLines={1} className="flex-1 text-[12px] font-sans-medium text-white">
                {period.topArtist}
              </Text>
            </View>
          </View>
          <View className="flex-1 rounded-xl border border-white/5 bg-white/[0.03] p-3">
            <Text className="text-[9px] uppercase tracking-widest2 text-white/35">Top genre</Text>
            <Text numberOfLines={1} className="mt-2 text-[12px] font-sans-medium text-white">
              {period.topGenre}
            </Text>
          </View>
          <View className="flex-1 rounded-xl border border-white/5 bg-white/[0.03] p-3">
            <Text className="text-[9px] uppercase tracking-widest2 text-white/35">Mood score</Text>
            <Text className="mt-2 text-[14px] font-serif italic text-white">{period.moodScore.toFixed(1)} / 10</Text>
          </View>
        </View>

        {period.tracks.length ? (
          <View className="mt-5">
            <Text className="mb-2.5 text-[9px] uppercase tracking-widest2 text-white/35">Top songs then</Text>
            <View className="gap-3">
              {period.tracks.map((track, i) => (
                <View key={`${track.title}-${i}`} className="flex-row items-center gap-3">
                  <Text className="w-4 text-[11px] text-white/25">{i + 1}</Text>
                  {track.albumImageUrl ? (
        ...
        {period.story ? (
          <View className="mt-5 flex-row items-start gap-2.5 border-t border-white/[0.06] pt-4">
            <View className="rounded-full border border-white/10 bg-white/[0.05] px-2 py-1">
              <Text className="text-[9px] uppercase tracking-widest2 text-white/55">{period.story.tag}</Text>
            </View>
            <Text className="flex-1 text-[12px] font-serif italic text-white/65">{period.story.line}</Text>
          </View>
        ) : null}
      </GlassCard>
    </MotiView>
