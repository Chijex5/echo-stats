          ...
          <View key={item.key} className="flex-row gap-4">
            <View className="items-center">
              <View
                ...
                style={{ borderColor: alpha.spotify(0.25), backgroundColor: alpha.spotify(0.08) }}
              >
                <Icon size={16} color={colors.echoGreen} />
              </View>
              {!isLast ? <View className="my-1 w-px flex-1" style={{ backgroundColor: alpha.white(0.08) }} /> : null}
            </View>
            <View className={isLast ? "flex-1 pb-0" : "flex-1 pb-5"}>
              <Text className="text-10 font-sans-semibold uppercase tracking-widest2 text-white/35">
                {item.title}
              </Text>
              <Text numberOfLines={2} className="mt-1.5 text-14 font-sans-semibold text-white">
                {item.value}
              </Text>
              <Text className="mt-1 text-12 text-white/40">{item.meta}</Text>
            </View>
          </View>
        );
      })}
